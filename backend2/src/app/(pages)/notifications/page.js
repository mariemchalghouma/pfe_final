...
const NotificationsPage = () => {
    ...
    const filteredNotifs = activeTab === 'Tous' 
        ? notifications 
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1 font-medium">{unreadCount} notification(s) non lue(s)</p>
                    </div>
                </div>
                ...
                {filteredNotifs.length > 0 ? (
                    <div className="divide-y divide-gray-50">
                        {filteredNotifs.map((notif) => (
                            ...
                            >
                                <div className={`mt-1 p-2.5 rounded-2xl shadow-sm ${notif.bgColor} ${notif.color}`}>
                                    <notif.icon className="text-xl" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3">
                                        <h3 className={`text-[16px] tracking-tight ${notif.isNew ? 'font-black text-gray-900' : 'font-bold text-gray-700'}`}>
                                            {notif.title}
                                        </h3>
                                        {notif.isNew && (
                                    ...
                                    <p className="text-[14px] text-gray-500 mt-1">{notif.message}</p>
                    ...
                    <div className="p-12 text-center flex flex-col items-center">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-1">Aucune notification</h3>
                        <p className="text-gray-500">Vous n&apos;avez aucune notification dans cette catégorie pour le moment.</p>
                    </div>
                )}
            </div>
        </div>
