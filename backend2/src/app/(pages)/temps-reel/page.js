...
const createTruckIcon = (status, selected) => {
  ...
  const color = selected ? '#f97316' : status === 'en_route' ? '#22c55e' : '#ef4444';
  return L.divIcon({
      ...
      <div className="w-[360px] bg-white border-r border-gray-200 flex flex-col min-h-0">
        <div className="p-4 border-b border-gray-100">
          ...
          <div className="mt-3 flex items-center gap-2 text-xs font-bold">
            ...
            <span className="px-2 py-1 rounded-lg bg-gray-100 text-gray-600">🚚 {stats.moving}</span>
            <span className="px-2 py-1 rounded-lg bg-gray-100 text-gray-600">⏸ {stats.stopped}</span>
          </div>
        </div>
          ...
          {!loading && filteredCamions.map((c) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${c.statut === 'en_route' ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-orange-50 text-orange-600 border border-orange-200'}`}>
      ...
      <div className="flex-1 relative min-h-0">
        <MapContainer center={[36.8, 10.18]} zoom={7} className="h-full w-full" style={{ height: '100%', width: '100%' }}>
          ...
          {clusters.map((cluster, idx) => {
            if (cluster.items.length === 1) {
              ...
              return (
                <Marker
                  ...
                  icon={createTruckIcon(truck.statut, selectedCamion?.camion === truck.camion)}
            ...
            return (
              <Marker key={`cluster-${idx}`} position={[cluster.lat, cluster.lng]} icon={createClusterIcon(cluster.items.length)}>
                <Popup>
