...
export default function SessionPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const valides = JSON.parse(
        localStorage.getItem(VALIDATION_STORAGE_KEY) || "[]",
      );
  ...
  useEffect(() => {
    const status = (data?.conversation?.validation_status || "")
      .toString()
      .toLowerCase()
      .trim();
    const isOk =
      status === "valide" || data?.conversation?.validation_conforme === true;
    if (isOk) {
      setIsValidated(true);
      setValidationNotice("");
      try {
        const valides = new Set(
          JSON.parse(localStorage.getItem(VALIDATION_STORAGE_KEY) || "[]"),
        );
    ...
    data?.conversation?.validation_status,
    data?.conversation?.validation_conforme,
  ]);
  ...
  const s = {
    ...
    messageAgent: { background: "#eff6ff", border: "1px solid #dbeafe" },
  ...
  const rapportSections = useMemo(
    () => parseRapport(conv.rapport),
    [conv.rapport],
  );
  ...
  const fuelSeries = useMemo(() => {
    ...
    const filtered = fuelData
      ...
      .filter(
        (row) =>
          row.ts != null && (callTimeMs != null ? row.ts <= callTimeMs : true),
      );
  ...
  const openFuelPointMap = (point) => {
    ...
    const timeLabel =
      point?.ts != null ? formatTime(point.ts) : point?.heure || "";
  ...
  const _convType = (conv.type_nc || conv.type || "")
    .toString()
    .toLowerCase()
    .trim();
  const showFuel =
    fuelSeries &&
    (_convType === "chute_carburant" ||
      _convType === "arret_et_chute_carburant");
  ...
  useEffect(() => {
    ...
    const convType = (conv.type_nc || conv.type || "")
      .toString()
      .toLowerCase()
      .trim();
    const allowed =
      convType === "chute_carburant" || convType === "arret_et_chute_carburant";
    if (!allowed) return;
    ...
    async function loadFuel() {
      try {
        ...
        const res = await fetch(
          `/api/carburant/${conv.camion_id}/niveau?date=${d}`,
        );
  ...
  useEffect(() => {
    ...
    async function loadPointNoir() {
      try {
        ...
        const dateKey = getDateKeyFromSource(
          sourceIdRaw,
          row.date_appel || row.ts_detection,
        );
  ...
  const dateLabel = useMemo(() => {
    ...
    try {
      return new Intl.DateTimeFormat("fr-FR", {
        dateStyle: "medium",
        timeStyle: "short",
      }).format(new Date(conv.date_appel));
    } catch {
      return String(conv.date_appel);
    }
  }, [conv.date_appel]);
  ...
  const handleValider = async () => {
    ...
    try {
      ...
      if (res?.ok === false) {
        ...
        const updatedPortes = Number(
          res?.updated?.voyagetracking_port_ouvert || 0,
        );
  ...
  if (!data)
    return (
      <div style={s.page}>Aucune conversation trouvée pour {session_id}</div>
    );
  ...
  const gaugePct =
    predictionPct != null ? Math.min(100, Math.max(0, predictionPct)) : 0;
  const gaugeDash = (gaugePct / 100) * gaugeCirc;

  return (
    <>
      <div style={s.page}>
        {/* inline styles removed to use app-standard inline styles/Tailwind */}
        ...
        {validationNotice ? (
          <div
            style={{
              ...s.notice,
              background:
                validationNoticeType === "error" ? "#fef2f2" : "#ecfdf5",
              color: validationNoticeType === "error" ? "#b91c1c" : "#065f46",
              borderColor:
                validationNoticeType === "error" ? "#fecaca" : "#a7f3d0",
            }}
          >
            {validationNotice}
          </div>
        ...
        <div style={s.header}>
          ...
          >
            <div style={s.tags}>
              ...
              <span style={{ ...s.tag, ...s.tagAlt }}>
                {conv.nb_tours ?? 0} tours
              </span>
              {pointNoirInfo && (
                <span style={{ ...s.tag, ...s.tagPointNoir }}>Point noir</span>
              )}
            </div>
            <button
              type="button"
        ...
        <div style={s.grid}>
          ...
          <div style={s.column}>
            <div style={s.card}>
              ...
              {audioSrc ? (
                <audio style={s.audioPlayer} controls src={audioSrc} />
              ) : (
            ...
            <div style={s.statsRow}>
              <div style={s.stat}>
                <div style={s.statLabel}>Duree</div>
                <div style={s.statValue}>{durationLabel}</div>
              </div>
              <div style={s.stat}>
                <div style={s.statLabel}>Tours</div>
                <div style={s.statValue}>{conv.nb_tours ?? 0}</div>
              </div>
              <div style={s.stat}>
                <div style={s.statLabel}>Session</div>
                <div style={s.statValue}>{session_id}</div>
              </div>
            </div>
            ...
            {showFuel && (
              <div style={s.card}>
                <div
                  style={{ ...s.cardTitle, justifyContent: "space-between" }}
                >
                  <span
                  ...
                  <span
                    style={{ fontSize: 12, fontWeight: 400, color: "#64748b" }}
                  >
                    Axe X: gps_dt
                  </span>
                </div>
              ...
              {predictionPct != null ? (
                <div>
                  ...
                  >
                    ...
                    >
                      <svg
                        ...
                        viewBox={`0 0 ${gaugeSize} ${gaugeSize / 2 + 10}`}
                      >
                        {/* Background arc */}
                        <path
                        ...
                        <path
                          d={`M ${gaugeStroke / 2} ${gaugeSize / 2} A ${gaugeRadius} ${gaugeRadius} 0 0 1 ${gaugeSize - gaugeStroke / 2} ${gaugeSize / 2}`}
                          fill="none"
                          stroke={predColor.fill}
                          strokeWidth={gaugeStroke}
                          strokeLinecap="round"
                          strokeDasharray={`${gaugeDash} ${gaugeCirc}`}
                          style={{
                            animation: "gaugeAnim 1.2s ease-out",
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        Probabilité que ce cas soit réellement non-conforme,
                        basée sur l&apos;analyse IA de la conversation.
                      </div>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    const problemeSection = rapportSections.find(
                      (sec) =>
                        sec.title.toLowerCase().includes("problème") ||
                        sec.title.toLowerCase().includes("probleme"),
                    );
                ...
                <div style={s.empty}>
                  Prédiction non disponible — le rapport n&apos;a pas encore été
                  généré.
                </div>
              )}
            </div>
              ...
              {rapportSections.length > 0 ? (
                ...
                >
                  {rapportSections.map((sec, i) => {
                    const isPrediction =
                      sec.title.toLowerCase().includes("prédiction") ||
                      sec.title.toLowerCase().includes("prediction");
                    return (
                      <div
                        key={i}
                        style={{
                      ...
                      >
                        <div
                          style={{
                            fontSize: 12,
                            fontWeight: 700,
                            color: isPrediction ? predColor.text : "#64748b",
                            marginBottom: 4,
                            display: "flex",
                            alignItems: "center",
                            gap: 6,
