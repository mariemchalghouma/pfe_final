...
const buildVoyageMapPoints = (voyageData, trajetPoints = []) => {
  ...
  const getLat = (pt) =>
    Array.isArray(pt) ? toNumber(pt[0]) : toNumber(pt.latitude ?? pt.lat);
  const getLng = (pt) =>
...
const SegmentPopover = ({ segment, position, onClose }) => {
  ...
  const duration =
    segment.duration ||
    Math.round((new Date(segment.end) - new Date(segment.start)) / 60000);
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ background: segColor.bg }}
            >
              {SegIcon && (
                <SegIcon className="text-white" style={{ fontSize: "12px" }} />
              )}
            </div>
            <div>
              <p
                style={{ fontWeight: 800, fontSize: "13px", color: "#1a1a2e" }}
              >
                {segColor.label}
              </p>
              <p
        ...
        <div className="space-y-1.5" style={{ fontSize: "12px" }}>
          {segment.type !== "driving" && (
            <div className="flex items-center gap-1.5">
              <FiClock
                style={{ color: "#9ca3af", flexShrink: 0, fontSize: "13px" }}
              />
          ...
          {segment.type === "driving" && (
            <div className="flex items-center gap-1.5">
              <FiActivity
                style={{ color: "#22c55e", flexShrink: 0, fontSize: "13px" }}
              />
          ...
          {segment.poiName && (
            <div className="flex items-center gap-1.5">
              <FiMapPin
                style={{ color: "#6366f1", flexShrink: 0, fontSize: "13px" }}
              />
                  ...
                  {segment.tempVar != null && (
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        style={{
                          fontWeight: 700,
                          color: segment.tempVar > 0 ? "#ef4444" : "#22c55e",
                        }}
                      >
                        Δ{segment.tempVar}°C
                      </span>
          ...
          {segment.type !== "ouverture_porte" && segment.tempOuv != null && (
            ...
            >
              ...
              <span style={{ fontWeight: 600, color: "#1a1a2e" }}>
                {segment.tempOuv}°C → {segment.tempFer}°C{" "}
                {segment.tempVar != null && `(Δ${segment.tempVar}°C)`}
              </span>
            </div>
          )}
        </div>
...
const GanttBar = ({
  ...
  return (
    <div className="flex items-stretch gap-0">
      ...
      <div
        className="w-[280px] flex-shrink-0 pr-3 py-3 border-r pl-3 cursor-pointer hover:bg-orange-50/40 transition-colors"
        style={{
      ...
      >
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          <FiUser
            style={{ color: "#9ca3af", fontSize: "11px", flexShrink: 0 }}
          />
          <span
      ...
      {/* Gantt bar */}
      <div
        className="flex-1 h-[42px] rounded-lg relative overflow-hidden mx-2 self-center"
...
const SidePanel = ({ data, onClose, onShowMap }) => {
  ...
  const drivingMin = segments
    .filter((s) => s.type === "driving")
    .reduce((s, seg) => s + (seg.duration || 0), 0);
  const stopMin = segments
    .filter((s) =>
      ["stop_conforme", "stop_non_conforme", "stop", "stop_long"].includes(
        s.type,
      ),
    )
    .reduce((s, seg) => s + (seg.duration || 0), 0);
  ...
  const primaryClient =
    data.clients?.find((c) => c?.client)?.client ||
    data.clients?.[0]?.code ||
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            <FiUser
              style={{ color: "#f97316", fontSize: "16px", flexShrink: 0 }}
            />
            <p
              className="truncate"
              style={{
          ...
          {chauffeurPhone && (
            <div
              ...
              style={{ color: "#64748b", flexShrink: 0, marginLeft: "8px" }}
            >
        ...
        >
          ...
          >
            ...
            >
              {drivingMin > 0 ? fmtDuration(drivingMin) : "0h00"}
            </p>
          </div>
          ...
          >
            ...
            >
              {stopMin > 0 ? fmtDuration(stopMin) : "0h00"}
            </p>
            <p
              style={{
                fontSize: "11px",
              ...
              >
                ...
                <div
                  style={{
                    ...
                    paddingBottom: "12px",
                    borderBottom:
                      i < segments.length - 1 ? "1px solid #f8fafc" : "none",
                  }}
                >
                  <div
                    className="flex items-center gap-2"
                  ...
                  >
                    ...
                    {!seg.poiName && seg.address && seg.address !== "—" && (
                      <span>{seg.address}</span>
                    )}
                    {!seg.poiName && (!seg.address || seg.address === "—") && (
...
const Camions = () => {
  ...
  const loadCamions = useCallback(async () => {
    ...
    try {
      ...
      const fetchAddressesSequentially = async () => {
        ...
        const toGeocode = camionsData.filter(
  ...
  const availableSites = useMemo(() => {
    const dynamicSites = new Set(
      ganttData
        .flatMap((d) => (d.clients || []).map((c) => normalizeSite(c.client)))
        .filter(Boolean),
    );
  ...
  const filteredGantt = useMemo(() => {
    ...
    return ganttData.filter((d) => {
      ...
      const matchesSite =
        ...
        d.clients?.some(
          (c) =>
            normalizeSite(c.client) === ganttSite ||
  ...
  const ganttStats = useMemo(() => {
    ...
    const totalClients = filteredGantt.reduce(
      (s, d) => s + (d.nbClients || 0),
      0,
    );
    return {
  ...
  const loadTrajet = useCallback(
    async (plaque, options = {}) => {
      ...
      try {
        const response = await camionsAPI.getCamionTrajet(
          plaque,
          date ? { date } : {},
        );
  ...
  const handleSelectCamion = useCallback(
    async (camion, options = {}) => {
  ...
  const handleClickCamionRow = useCallback(
    async (voyageData) => {
      ...
      if (cam) {
        trajetPoints = await handleSelectCamion(cam, {
          date: ganttDate,
          color: "#f97316",
        });
      } else {
        trajetPoints = await loadTrajet(voyageData.camion, {
          date: ganttDate,
          color: "#f97316",
        });
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              >
                {ganttStats.totalClients} client
                {ganttStats.totalClients > 1 ? "s" : ""}
              </span>
              <span
                style={{
                  padding: "5px 12px",
              ...
              >
                {ganttStats.totalCamions} camion
                {ganttStats.totalCamions > 1 ? "s" : ""}
              </span>
            </div>
          </div>
            ...
            >
              ...
              <div>
                {filteredGantt.map((d, i) => (
                  <div
                    key={d.id || `${d.camion}-${d.voycle}-${i}`}
                    style={{
                      borderBottom:
                        i < filteredGantt.length - 1
                          ? "1px solid #f9fafb"
                          : "none",
                      padding: "4px 8px",
                      transition: "background 0.15s",
                      background:
                        selectedVoyageId === d.id ? "#fff7ed" : "transparent",
                    }}
              ...
              {filteredGantt.length === 0 && !ganttLoading && (
                <div
                  ...
                  style={{ padding: "80px 0", color: "#d1d5db" }}
                >
                  <FiBarChart2
